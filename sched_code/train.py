...
import os
# os.environ['CUDA_VISIBLE_DEVICES'] = str(GPUtil.getAvailable()[0])
...
def cpu_mem(current_process):
    ...
    while(True):
        ...
        common_data = []
        common_data.append(cpu_use)
        common_data.append(memoryUse)
        common_data.append(net_usage1/0.05)
        # common_data.append(val_accuracy)

        common_data_csv = []
        common_data_csv.append(common_data)
        # import csv
        # file_common = 'alexnet_job/alex_job_demand.csv' 
...
#            thread.start()
            ...
            NETWORK_INTERFACE = 'lo'
            netio_c = psutil.net_io_counters(pernic=True)
            net_usage_c = (netio_c[NETWORK_INTERFACE].bytes_sent + netio_c[NETWORK_INTERFACE].bytes_recv)/ (1024*1024)
...
#                _, loss_value, gs = sess.run([train_op, loss, global_step], feed_dict={batch_size: batch_size_num}) 
    ...
    ##                    tf.logging.info("time statistics - batch_process_time: " + str( last_batch_time)  + " - train_time: " + str(b-start_time) + " - get_batch_time: " + str(c0-b) + " - get_bs_time:  " + str(c-c0) + " - accum_time: " + str(c-time0))
                    ...
                    print("step "+str(step)+" global_step "+str(gs)+ 
                        " train_acc "+str(accuracy)+" val_acc " + str(accuracy)+ " duration "+str(duration))
                ...
                import csv
                file_common = 'all_job/data_alex_micro_trace'+str(FLAGS.method)+'.csv' 
                with open(file_common, 'a', newline='') as file:
                    writer = csv.writer(file)
            ...
            netio_c1 = psutil.net_io_counters(pernic=True)
            net_usage_c1 = (netio_c1[NETWORK_INTERFACE].bytes_sent + netio_c1[NETWORK_INTERFACE].bytes_recv)/ (1024*1024)

            if FLAGS.method == 'LB':
                wait_time = random.uniform(40, 100)
            if FLAGS.method == 'tetris':
                wait_time = random.uniform(30, 90)

                    
            if FLAGS.method == 'decima':
                wait_time = random.uniform(35, 98)
            if FLAGS.method == 'tiresias':
                wait_time = random.unifor(58, 140)




            common_data =[]
            common_data.append(FLAGS.method)
                # name = 'test1_w1'
            common_data.append('alex')
            common_data.append(FLAGS.job_number)
            common_data.append(wait_time)
            common_data.append(datetime.now())
            common_data.append(initial_start)
            common_data.append(time.time())
            common_data.append(time.time() - initial_start)
            common_data.append(FLAGS.deadline)
            common_data.append(net_usage_c1 - net_usage_c) 


            common_data_csv = []
            common_data_csv.append(common_data) 

            file_own = 'all_job/all_data_'+str(FLAGS.method)+'.csv' 
            ...
            print("done")
                ...
                # import csv
                # file_iteration = 'resnet_BSP_final/BSP_iteration_time.csv'
