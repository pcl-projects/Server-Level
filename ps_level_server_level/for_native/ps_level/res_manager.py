...
import res_limiter_server as rls

__WORKER_NUM = 10


__BANDWD_MAX_SV_RES = 13.0 * 1024 / 2  # Max server resource for bandwidth, Mbit
__CPU_MAX_SV_RES = (5 * 2 + 5 + 3) * 500.0 / 2  # Max CPU resource for bandwidth, Mbit, 500 for each core, total
...
# For simulation of PS-level self-managing
# Degrade from server-level to PS-level
def ava_server_res_checker_thread():
    ...
    while True:
        ...
        time.sleep(60)  # For simulation of PS-level self-managing
...
def run_res_manager(listening_ip, listening_port):
    ...
    res_limiter_server_t = threading.Thread(target=rls.run_res_limiter_server)
    ava_server_res_checker_t = threading.Thread(target=ava_server_res_checker_thread)
    listener_t.start()
    res_limiter_server_t.start()
    ava_server_res_checker_t.start()
    listener_t.join()
    res_limiter_server_t.join()
    ava_server_res_checker_t.join()


if __name__ == "__main__":
    run_res_manager("0.0.0.0", 20103)
