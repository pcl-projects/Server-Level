...
def job_timer_thread(q: queue.Queue):
    global __RE_JC_NAME
    ...
    while True:
        ...
        for job_name in jobs_to_add:
            __ACTIVE_JOBS[job_name] = time.time() - __STRAGGLER_PERIOD + 6
        for job_name, reset_time in __ACTIVE_JOBS.items():
            last_time = time.time()
            if last_time - reset_time >= __STRAGGLER_PERIOD - 3:
                __ACTIVE_JOBS[job_name] = time.time()
                q.put(__get_jc_names_by_job_name(job_name))
        time.sleep(10)
...
def run_res_limiter():
    q = queue.Queue()
    t = threading.Thread(target=job_timer_thread, args=(q,))
    t.start()
    while True:
        jc_names = q.get()
        print("Res limiter timer:", jc_names)
        for jc_name in jc_names:
            if "-ps" in jc_name and jc_name in gd.ACTIVE_PSS:
                print("Limit active PS:", jc_name)
                if int(jc_name[-1]) == 1:
                    prob = 101
                else:
                    prob = -1
                if jc_name in gd.LIMITED_JCS:
                    gd.LIMITED_JCS.pop(jc_name)
                unlimit_bandwidth(jc_name)
                unlimit_cpu(jc_name)
                if __probability(prob):
                    gd.LIMITED_JCS[jc_name] = [None, None]
                    if random.randint(0, 1) < 2:
                        limit_bandwidth(jc_name, 500)
                        gd.LIMITED_JCS[jc_name][0] = 500
                    else:
                        limit_cpu(jc_name, 30)
                        gd.LIMITED_JCS[jc_name][1] = 30
                        print(
                            f"------------------Limit JCS: {gd.LIMITED_JCS} ---------------------------"
                        )
                break
