...
class ParameterServer(object):
    def __init__(self, job_name, scheduler_rref, num_ps, num_idle_ps, num_workers, id, worker_rrefs, lr):
        self.logger = Logger(job_name=job_name, file_dir=f"./measurement/logs/{job_name}_ps{id}.log").logger
        ...
        self.res_manager_connm = SocketMsger.tcp_connect("172.17.0.1", 20003)
...
class Worker(object):
    def __init__(
        ...
        self.logger = Logger(job_name=job_name, file_dir=f"./measurement/logs/{job_name}_worker{id}.log").logger
        self.id = id
        self.num_ps = num_ps
        self.num_idle_ps = num_idle_ps
        self.num_workers = num_workers
    ...
    def run_worker(worker_rref, ps_rrefs, worker_rrefs):
        self: Worker = worker_rref.local_value()
        ...
        for epoch in range(self.num_epochs):
            for batch_idx, (data, target) in enumerate(train_loader):
                ...
                if self.reassigned_flag == 1:
                    ...
                    if self.id == 0:
                        para_reassign_time = 1000 * (time.time() - para_reassign_time_0)
                        self.logger.info(f"Para reassign time {para_reassign_time:.3f} ms")
...
class Scheduler(object):
    def __init__(self, job_name, num_ps, num_idle_ps, num_workers, model):
        self.logger = Logger(job_name=job_name, file_dir=f"./measurement/logs/{job_name}_scheduler.log").logger
        self.num_ps = num_ps
        self.num_idle_ps = num_idle_ps
        self.num_workers = num_workers
        self.active_ps = [i for i in range(num_ps)]
...
def get_accuracy(worker_rref, data_dir, test_batch_size, job_name):
    logger = Logger(job_name=job_name, file_dir=f"./measurement/logs/{job_name}_acc_tester.log").logger
...
def run(job_name, rank, num_ps, num_workers, data_dir, model, batch_size, lr, num_epochs):
    ...
    if rank == 0:
        ...
        futs = []
        futs.append(
            rpc.rpc_async(to="acc_tester", func=get_accuracy, args=(scheduler.worker_rrefs[0], data_dir, 128, job_name))
        )
        for worker in range(num_workers):
            futs.append(
                rpc.rpc_async(
                    ...
                    args=(scheduler.worker_rrefs[worker], scheduler.ps_rrefs, scheduler.worker_rrefs,),
                )
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", "--data_dir", type=str, default="./measurement/data", help="The location of dataset.")
